...
For example, if an author claims that two groups are different (or not) based on an erroneously reported significant test, we consider this claim to potentially have theoretical consequences.
...
Inversely, `{r} mismatches` cases mismatched the erroneously reported p-value (`{r} round(((mismatches / (matches+mismatches)) * 100),0)`%), possibly representing genuine typos, e.g. the authors interpreted the results in line with the computed p-value and not the erroneously reported p-value.
