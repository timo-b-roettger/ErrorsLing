...
To gain some insight, we approximate an answer to this questions as follows: First, we extracted a random sub sample (15%, n = `{r} nrow(xdata_sample)`) of articles which contain at least one decision inconsistencies, as equally distributed across publication years as possible.
We restricted the sub sample to only those decision inconsistencies that involve either a larger (\>) or small (\<) comparison of the p-value to allow unambiguous interpretation of the results.
