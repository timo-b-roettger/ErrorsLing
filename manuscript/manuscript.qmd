...
The question arises as to whether the inconsistencies theoretically matter.
...
To gain some insight, we approximate an answer to this questions as follows: First, we extracted a random sub sample (15%, n = `{r} nrow(xdata_sample)`) of articles which contain at least one decision inconsistency, as equally distributed across publication years as possible.
...
Out of the `{r} nrow(xdata_sample)` assessed cases, most were straightforwardly interpretable `{r} matches+mismatches` (`{r} round((((matches+mismatches) / nrow(xdata_sample)) * 100),0)`%). We coded `{r} unclear` cases as unclear, i.e. even after assessing the manuscript in detail, it remained unclear to us how the p-value was interpreted.
...
The present examination did not indicate any noticeable differences in inconsistency rates across journals or publication years.
